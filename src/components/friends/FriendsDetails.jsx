...
import { AiOutlinePhone, AiOutlineMessage, AiOutlineVideoCamera, AiOutlineClockCircle, AiOutlineInbox, AiOutlineDelete } from 'react-icons/ai';
...
const recentInteractions = [
    ...
    { type: "Video", note: "Caught up on life updates", date: "Jan 14, 2026" },
    { type: "Text", note: "Shared a job opportunity", date: "Dec 30, 2025" },
];

function InteractionIcon({ type }) {
    const cls = "w-4 h-4 text-gray-400 shrink-0";
    if (type === "Text") return <AiOutlineMessage className={cls} />;
    if (type === "Video") return <AiOutlineVideoCamera className={cls} />;
    if (type === "Meetup") return <AiOutlinePhone className={cls} />;
    return <AiOutlineMessage className={cls} />;
}
    ...
    return (
        <div className="min-h-screen bg-[#f4f7f5] px-4 py-10">
            <div className="max-w-4xl mx-auto flex flex-col gap-5">
                ...
                <button
                    onClick={() => navigate(-1)}
                    className="self-start text-sm text-[#1e4d3b] hover:underline bg-transparent border-0 cursor-pointer flex items-center gap-1"
                >
                    ← Back
                </button>
                    ...
                    <div className="flex flex-col gap-4 w-full md:w-64 shrink-0">
                        ...
                        <div className="bg-white rounded-2xl border border-gray-100 p-6 flex flex-col items-center gap-3 text-center">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-gray-400 mt-0.5">{friend.email}</p>
                            </div>
                    ...
                    <div className="flex flex-col gap-4 flex-1">
                        ...
                        <div className="bg-white rounded-2xl border border-gray-100 p-5">
                            ...
                            <div className="grid grid-cols-3 gap-3">
                                {[
                                    { label: "Call", Icon: AiOutlinePhone },
                                    { label: "Text", Icon: AiOutlineMessage },
                                    { label: "Video", Icon: AiOutlineVideoCamera },
                                ].map(({ label, Icon }) => (
                                    <button
                                        key={label}
                                        className="flex flex-col items-center gap-2 py-4 rounded-xl border border-gray-100 hover:border-[#1e4d3b] hover:bg-[#f0f9f4] transition-all cursor-pointer bg-transparent"
                                    >
